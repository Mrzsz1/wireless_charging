...
- `qa/understanding.rs` owns `ConversationResolver`, `ResolvedQuestion`, `ResearchQuery`, `ResearchIntent`, `IntentRouter`, and `ExecutionMode`. For a contextual reference with trusted history, any provider advertising the understanding capability may make one bounded Provider-native `qa-understanding-v1` call before retrieval; self-contained turns skip that call. Invalid, timed-out, unavailable, budget-rejected, or schema-rejected output falls back to deterministic resolution and routing with explicit telemetry. Offline evidence mode uses the deterministic path.
...
- `ResearchSessionState` is derived deterministically from trusted user turns plus the current question and is injected as structured JSON beside recent exchanges. It tracks the active problem, latest objectives, constraints, assumptions, methods, paper aliases, hypotheses, open questions, source message IDs, and a monotonic revision. Explicit replace/remove language updates state rather than merely appending stale values. Its serialized size is included in the context budget; `QaRunManifest` records version, revision, and objective/constraint counts.
...
- A request-scoped `LlmBudgetGuard` gates understanding/resolver, query planner, generator, and any future model verifier call before provider execution. Reservations enforce both call and token-cost ceilings; successful calls settle to prompt + returned-output token estimates. Rejection is stable (`LLM_BUDGET_EXCEEDED`) and fail-soft only where the stage already has a deterministic fallback.
- `QaRunManifest` records policy ceilings separately from `routingLlmCallsUsed`, settled/reserved token cost, executed stage names, and rejection reasons. Budget telemetry reflects executed reservations, not a copied policy constant, and the QA sidebar exposes the used/maximum values.
...
- Both capable providers use the same `PlanningProvider::complete_structured` boundary for two possible short-lived calls: a pre-retrieval understanding call receives the bounded current question and trusted recent history, while the RetrievalContract call after baseline retrieval receives only the resolved question and bounded candidate summaries. Codex uses `codex exec --output-schema`; Compatible API uses a non-streaming `response_format=json_schema` request against the configured endpoint. Both emit no UI answer tokens, validate the same closed schemas, consume the request budget, and fail soft to deterministic behavior.
...
- `heldout_questions.json` remains pending until an independent curator freezes at least 30 cases and seals the canonical cases array. Every run must contain complete answer claims, EvidenceItems, stable source IDs, and checksums; every claim requires two distinct blinded independent reviews and every disagreement requires a third distinct adjudicator. Missing, duplicate, mismatched, non-independent, or tampered data fails closed.
- Held-out verdicts are `supported | partially_supported | unsupported | contradicted | not_applicable | not_verifiable`. Claims may be classified as `factual | reference | method | constraint`; the evaluator reports factual precision with Wilson 95% CI, unsupported/contradicted/not-applicable rates, citation ID precision/completeness, reference support, method recall, and critical-constraint preservation.
...
- Production retrieval MRR is canonical-document MRR. Duplicate passages collapse before rank calculation, and a Wiki source plus its paired `wiki:sources/<source-id>` / `paper:sources/<source-id>` primary paper collapse to the same research-work identity. Passage MRR remains a diagnostic only. `mrr-diagnostics-latest.json` records first relevant passage/document ranks, stable and canonical IDs, channel membership, RRF/base/cross/final scores, and document-repeat penalties without paths or source text.
