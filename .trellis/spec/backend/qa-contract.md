...
- `EvidenceAvailability` deterministically classifies `paper | book | wiki` as support eligible and graph-only navigation as ineligible. Zero usable evidence means no selected evidence or an all-Graph set; a support-eligible set with a required-facet gap is partial coverage, not zero evidence. No retrieval score, top-K, grounding threshold, call budget, or token ceiling changes to manufacture zero evidence.
