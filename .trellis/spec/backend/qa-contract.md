...
- Every assistant message stores schema/prompt/retriever/context versions, provider, requested/resolved model, temperature where applicable, output/context limits, prompt SHA-256, index snapshot SHA-256, recent/compacted/coreference-resolved history IDs, resolver/router/planner status and aggregate latency, structured research intent and execution mode, fallback reasons, evidence checksums, context budget, repair record, and completeness result in `run_manifest`.
