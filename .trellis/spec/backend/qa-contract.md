...
- Canonical intent literals are exactly `solve | novelty | relationship`. Every downstream weighting, diversity, and evidence-retention branch must match these exact values; `solution` is not an alias. When a `solve` or `novelty` query recalls a `method` candidate, final evidence selection keeps at least one method.
