...
- A measured product gate failure is a valid E2E result and must be written to the report before exit `2`; it must not be converted to runner success. `QA_REAL_E2E_CASE_ID` may select one public case for diagnosis. Report v4 separates `scope`, `executedScopePassed`, `fullSuiteEvaluated`, and `releaseEligible`: a passing selected case sets `passed=true` / exit 0 while `releaseEligible=false`; only an executed and passing complete five-case suite is release eligible. Expected executed-scope failure exits 2, while environment/provider/file errors exit 1.
