...
- Exploratory and research queries pass through `ProblemParser` and `MethodMatcher`. The representation preserves domain, objectives, constraints, assumptions, decision variables, and related problem types; matching returns methods with rationale and required conditions. Only research/exploratory modes add bounded related-problem/method terms to retrieval. DirectQA ranking remains unchanged. Parser/matcher versions, status, representation summaries, and candidate methods are persisted in `QaRunManifest`.
