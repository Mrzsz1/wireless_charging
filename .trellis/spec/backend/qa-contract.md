...
- `AdaptiveRoutingPolicy` maps `DirectQA`, `ResearchQA`, and `ExploratoryResearch` to bounded retrieval rounds, queries, candidates, LLM-call budgets, and token ceilings. Direct executes at most one retrieval round and never invokes the query planner, even when a callback is available. Coverage can stop any path early, while exploratory mode is capped at three rounds.
- A request-scoped `LlmBudgetGuard` gates understanding/resolver, query planner, generator, and semantic verifier before provider execution. Token admission is exactly `tokenCostUsed + tokenCostInFlight + newReservation <= tokenCostCeiling`; historical cumulative reservations never participate. `tokenCostUsed` is actual cost committed by completed reservations, while `tokenCostInFlight` is the maximum cost held by active reservations. LLM call admission remains cumulative and is never refunded by token settlement or release. Rejection stays stable as `LLM_BUDGET_EXCEEDED: <stage>:<call_budget|token_budget>` and remains fail-soft only where the stage already has a deterministic fallback.
- `LlmBudgetGuard::reserve(stage, ceiling)` returns a unique non-clone `LlmReservation`. Consuming `settle(actual)` removes that exact active ID, releases its in-flight ceiling, and adds actual cost to used; consuming `release()` removes it without adding used. An unclosed handle releases in `Drop`, so pre-provider errors, early `?`, cancellation unwind, and task panic cannot leak in-flight capacity. A closed ID rejects a second internal close without changing counters. `reconfigure()` replaces only the policy and preserves calls, used, in-flight, historical reserved totals, stages, and rejections.
- `qa-run-v21` records `routingTokenCostCeiling`, `routingTokenCostUsed`, `routingTokenCostInFlight`, historical compatibility field `routingTokenCostReserved`, explicit alias `routingTokenCostReservedTotal`, `routingLlmCallsUsed`, stages, and rejection reasons. The two reserved fields are audit-only historical cumulative maxima and remain equal; old manifests default new fields to zero. The QA sidebar exposes used/in-flight/ceiling/history separately.
...
- `qa-heldout-run-v2` first projects each verified claim through the same pure status-to-visible-text mapping already used by production AnswerRepair: contradicted/not-verifiable claims use the existing fixed replacement sentence, partially supported claims use the fixed “未支持部分已省略” system notice without retaining the original claim, and other statuses retain their text. It then applies the same pure canonical transformation used by `natural-markdown-v2`: existing appendix removal, boundary trim, `[E<digits>]` removal, unsafe Markdown-link target sanitation, Windows absolute-path redaction, and final trim. This is a behavior-preserving extraction and does not change verifier decisions or the final QA answer. Because atomic claim extraction intentionally masks Markdown link destinations and code/math literals, a non-literal semantic projection may only recover an exact final Markdown source span by matching its remaining visible chunks in order within one line; cross-line or oversized gaps fail closed. Adjacent or overlapping claims collapsed by AnswerRepair may share an earlier span only when the complete projected text is an exact final-answer match. Every exported claim must remain non-empty and occur verbatim in the final answer, and `answerClaims.len == runManifest.answerCompleteness.claimCount`. `citedEvidenceIds` remains the structured provenance copied from `runManifest.claimVerifications.evidenceIds`; visible claim text need not contain `[E#]`. Rust and Python independently require every cited ID to belong to the run's complete EvidenceItem ID set, while production checksum IDs/digests must still match that full set. Case files use same-directory `.json.part` + fsync + rename; the whole run directory is renamed from `.part` only after all cases pass. A failed partial is never read, overwritten, deleted, or promoted as success and may only be preserved as an `invalid_harness_run` record. Runner output contains no reviewer verdict and tests use only synthetic/development text, never the sealed heldout candidate pool.
...
- Grounding is a two-object contract: `verify_and_repair_with_semantic(...) -> (repaired_answer, draft_report)` records Draft Audit, then `audit_repaired_answer(repaired_answer, evidence, draft_report) -> FinalGroundingAudit` performs deterministic Final Audit without another LLM. Persistence, trusted history, and `CitationValidation.supported` read only Final Audit; Draft counts remain diagnostics and are never copied back into the final gate.
- A final factual claim passes only when its whitespace/repair-boundary-punctuation-normalized text and exact evidence-ID list map to a Draft `Supported` claim, every ID belongs to the current evidence set, and at least one cited item is non-Graphify. New or unmatched final facts fail closed. `is_grounding_system_notice` is shared by extraction and trusted-history filtering; fixed repair notices are neither factual claims nor citation obligations. When no supported factual claim remains, the visible result is the fixed insufficiency sentence, final status is `insufficient_supported_claims`, persistence is rejected, and trusted history is empty.
- `qa-prompt-v15` makes `executionMode=direct` override the full research profile: Direct asks for 1–3 minimal supported facts plus an evidence boundary. Research/Exploratory receives deterministic evidence-coverage labels and must omit or use a fixed system notice for uncovered profile elements. Natural-answer completeness requires the grounded claim minimum but does not force ungrounded profile labels; structured output continues to require its full role contract.
...
- QA lifecycle logging uses `qa-trace-v1` typed events for prepare/generate/semantic/audit/persist/E2E boundaries. Desktop files use Tauri AppLogDir in debug and release; development E2E uses Git-ignored `apps/desktop/logs/`. Logs exclude questions, answers, prompts, claims, snippets, paths, credentials, tokens, provider payloads, and reasoning.
...
- Deterministic tests assert Draft-invalid/Final-valid persistence, new-final-fact rejection, system-notice classification, Final Audit idempotence, partial-claim removal, no-supported insufficiency, Direct contract priority, repair-boundary punctuation normalization, the exact five-category/source/hash fixture contract, shared UI/runner core entrypoints, temporary workspace cleanup, metadata redaction, pre/final verdict separation, persistence-failure `final=null`, hashed claim diagnostics, unknown-citation rejection, generator-budget rejection, and zero-evidence semantic-not-run handling.
...
temp E2E adapter+
                +-> v3 draft/final metadata-only report -> explicit PASS or measured FAIL
```
