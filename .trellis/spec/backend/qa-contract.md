...
- `AdaptiveRoutingPolicy` maps `DirectQA`, `ResearchQA`, and `ExploratoryResearch` to bounded retrieval rounds, queries, candidates, LLM-call budgets, and token ceilings. Direct defaults to no planner and lower query/candidate/call/token budgets; an explicitly supplied legacy planner callback remains an interface-compatible override. Coverage can stop any path early, while exploratory mode is capped at three rounds. Policy version and all cost ceilings are auditable in `QaRunManifest`.
