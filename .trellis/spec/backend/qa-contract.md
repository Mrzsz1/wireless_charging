...
- `qa-heldout-run-v2` projects each pre-render verified claim through the same pure canonical visible-text transformation used by `natural-markdown-v2`: existing appendix removal, boundary trim, `[E<digits>]` removal, unsafe Markdown-link target sanitation, Windows absolute-path redaction, and final trim. Every projected claim must remain non-empty and occur verbatim in the final answer, and `answerClaims.len == runManifest.answerCompleteness.claimCount`. `citedEvidenceIds` remains the structured provenance copied from `runManifest.claimVerifications.evidenceIds`; visible claim text need not contain `[E#]`. Rust and Python independently require every cited ID to belong to the run's complete EvidenceItem ID set, while production checksum IDs/digests must still match that full set. Case files use same-directory `.json.part` + fsync + rename; the whole run directory is renamed from `.part` only after all cases pass. A failed partial is never read, overwritten, deleted, or promoted as success and may only be preserved as an `invalid_harness_run` record. Runner output contains no reviewer verdict and tests use only synthetic/development text, never the sealed heldout candidate pool.
