...
- Bundle validation requires every claim text to occur verbatim in the final answer, `answerClaims.len == runManifest.answerCompleteness.claimCount`, local `[E#]` markers to match cited IDs, and production evidence checksum IDs/digests to match the full evidence set. Case files use same-directory `.json.part` + fsync + rename; the whole run directory is renamed from `.part` only after all cases pass. Runner output contains no reviewer verdict and tests use only synthetic/development text, never the sealed heldout candidate pool.
