...
- [ ] 对清理后的提交再次运行秘密扫描和 `git diff --cached --check`。

## C. 私有 GitHub 发布

- [ ] 提交安全配置与任务文档。
- [ ] 创建或复用私有 GitHub 仓库，设置 `origin` 并推送默认分支。
- [ ] 使用 GitHub CLI/API 确认可见性为 PRIVATE、默认分支存在且 upstream 正确。
- [ ] 更新任务结果，归档并记录 Trellis journal。
