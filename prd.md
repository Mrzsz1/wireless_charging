...
| 39 | 文献库内容分类（2026-08-12） | 文献库按受控页面类型提供中文分类筛选；`map` 统一面向用户显示为“知识地图”，方法库只展示 method 页面；分类与搜索、年份、状态、排序和分页联动 |
...
#### 13.22 文献库内容分类与客户端 0.12.2（2026-08-12）

文献库由混合页面流升级为按知识角色浏览的分类视图：

1. 分类栏提供“全部、论文文献、方法、综述、概念、系统模型、优化目标、数据与实验、研究问题、知识地图”，每项显示当前结果集数量。
2. `map` 是 Wiki 的导航/知识地图页，不是论文类型；客户端统一显示“知识地图”，并使用独立图标，避免与论文文献混淆。
3. 分类在搜索、年份、状态与排序之后执行，并在分页之前执行；切换分类、搜索条件或页大小时回到第一页，结果收缩时页码自动收敛。
4. “方法库”入口固定只展示 `method` 页面，不显示通用分类栏；未知页面类型仍保留在“全部”中并标记为“其他页面”，避免静默丢失。
