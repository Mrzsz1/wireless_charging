...
硬规则：自动发现结果不得直接进入 wiki；手动投放也不得绕过 canonical 的可读性与状态检查。
...
| 用户已配置的论文自动发现任务 | **是，仅可写 inbox 候选**；不得自动晋升、编译或形成 novelty 结论 |
...
1. 系统或用户：论文进入 `auto-discovered` 或 `manual-drop`，写明采集来源。  
2. 用户：triage；选择项晋升 `raw/canonical/`，保留 provenance。  
...
1. 建立 Windows 桌面客户端 P0：工程骨架、知识库目录选择、Markdown/frontmatter 解析、SQLite FTS5 派生索引。  
2. 完成 P1 阅读与检索：库水位、全局搜索、文献/方法详情、wikilink 与反向链接。  
3. 完成核心书籍、PDF 页码定位、Graphify 图谱和多文献对比。  
4. 接入 Luna 自然语言问答与可审计证据面板。  
5. 接入现有发现、下载、A 编译、Lint、Graphify 更新流程，并打包 Windows 安装程序。  
