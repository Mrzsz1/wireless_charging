...
Input: The set of PoIs O, the farthest charging distance D, the error threshold  $\epsilon$ , and the constant  $\beta$ 

Output: All MCSs and the candidate subarea set  $\Gamma$ 

1: for each PoI  $o_{i} \in O$  do
...
Input: The number of chargers M, all candidate MCSs and their corresponding candidate positions set  $\Gamma$ , the set of PoIs O, and the objective function  $P_{addit}(C)$ 
