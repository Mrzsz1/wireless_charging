...
pub fn required_answer_section_contract(intent: &str) -> Vec<AnswerSectionContract> {
    let values: &[AnswerSectionContract] = match intent {
        "literature" => &[
...
pub fn required_answer_role_contract(intent: &str) -> Vec<AnswerRoleContract> {
    let values: &[AnswerRoleContract] = match intent {
        ...
        "solution_search" => &[
            AnswerRoleContract {
                id: "problem_classification",
                title: "问题分类",
            },
            AnswerRoleContract {
                id: "candidate_methods",
                title: "候选方法",
            },
            AnswerRoleContract {
                id: "applicability",
...
pub fn validate_answer_completeness(
    ...
    let missing_sections = if structured_roles.is_some() {
        required_sections
            .iter()
    ...
    let missing_elements = if let Some(observed_roles) = structured_roles {
        required_answer_role_contract(intent)
            .into_iter()
...
pub fn build_run_manifest(
    ...
    QaRunManifest {
        ...
        routing_confidence: context.retrieval_query.routing_confidence.clone(),
        resolver_escalated: context.retrieval_query.resolver_escalated,
        ...
        method_hypotheses: context.retrieval_query.method_hypotheses.clone(),
        discovered_methods: context.retrieval_query.discovered_methods.clone(),
        corroborated_method_hypotheses: context
            .retrieval_query
...
mod tests {
    ...
    fn natural_answer_contract_has_no_fixed_sections_and_requires_internal_evidence_ids() {
        ...
        for required in [
            ...
            "## 模型补充（可能不准确）",
            "研究建议必须用",
            "直接事实问题优先只回答 1–3 条最小充分事实",
            "直接复用 evidence snippet 的主语、谓语、核心术语和限定词",
            "保留支撑结论的简短英文关键短语",
        ] {
    ...
    #[test]
    fn specialized_natural_profiles_have_distinct_completeness_contracts() {
        let profiles = [
            "method_improvement",
            "solution_search",
            "problem_modeling",
        ...
        for profile in profiles {
            let roles = required_answer_role_contract(profile);
            assert!(roles.len() >= 5, "{profile}");
            let answer = roles
                .iter()
        ...
        assert_ne!(
            ...
            required_answer_elements("problem_modeling")
        );
    }
