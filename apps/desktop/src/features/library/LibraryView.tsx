...
export function LibraryView({ query, results, catalog, pageType = 'source', filters, loading, onQueryChange, onFiltersChange, onOpenResult }: LibraryViewProps) {
  ...
  return <section className="library-view">
    ...
    <div className="library-meta" data-testid="library-search-status"><span>{loading ? '正在搜索…' : query ? `找到 ${results.length} 条结果` : `已加载 ${catalog.length} 个页面`}</span><span>仅当前知识库</span></div>
    <div className="library-results" data-testid="library-search-results">{visibleResults.map((result) => <button className="library-result" key={result.id} onClick={() => onOpenResult(result)}><div className="result-type">{result.pageType === 'method' ? <Gauge size={15} /> : result.pageType === 'synthesis' ? <BookOpen size={15} /> : <FileText size={15} />}<span>{typeLabels[result.pageType] ?? result.pageType}</span></div><div className="result-main"><strong>{result.title}</strong><span>{result.year || '年份未记录'} · {result.sourcePath}</span><p>{result.snippet || result.summary}</p></div><ExternalLink size={15} className="result-open" /></button>)}{!loading && !visibleResults.length && <div className="library-empty"><FileText size={30} /><strong>{query ? '没有找到匹配页面' : '当前筛选没有页面'}</strong><span>{query ? '尝试更换关键词，或先在设置中选择知识库目录。' : '调整年份或状态筛选后重试。'}</span></div>}</div>
  </section>
}
