...
export function GraphView({ onOpenPage, refreshVersion = 0, targetNodeId = '' }: GraphViewProps) {
  ...
  return <section className="graph-view" data-testid="graph-view" data-refresh-version={refreshVersion}>
    <div className="library-heading"><div><h1>知识图谱</h1><p>从社区概览逐步展开局部关系，所有连线均标注为派生关系。</p></div><div className="graph-counts"><span>{graph?.nodeCount ?? 0} 节点</span><span>{graph?.edgeCount ?? 0} 边</span><span>{graph?.communityCount ?? 0} 社区</span></div></div>
