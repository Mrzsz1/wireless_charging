...
export function CoreBooksView({ onOpenLink, target }: CoreBooksViewProps) {
  ...
  return <section className="books-view" data-testid="books-view">
    <div className="library-heading"><div><h1>核心书籍</h1><p>两本算法专著、61 个章节、1171 个 PDF 物理页。</p></div><div className="book-quality-badge"><ShieldCheck size={15} />检索质量 ≥95%</div></div>
    ...
    <div className="book-reader-layout">
      ...
      <main className="book-markdown-column" data-testid="book-chapter-content">{selectedChapter ? <><div className="book-chapter-heading"><div><h2>{selectedChapter.chapter.title}</h2><p>{pageLabel(selectedChapter.chapter.physicalPageStart)} – {selectedChapter.chapter.physicalPageEnd ?? '未记录'} · {selectedChapter.chapter.charCount.toLocaleString()} 字符</p></div></div><MarkdownReader body={selectedChapter.body} sourcePath={selectedChapter.chapter.markdownPath} onOpenLink={onOpenLink} /></> : <div className="book-empty"><BookOpen size={28} /><strong>选择一个章节开始阅读</strong></div>}</main>
