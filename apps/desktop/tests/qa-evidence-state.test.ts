...
test('natural answers report backend appendix integrity instead of claim coverage', () => {
  ...
  assert.equal(summary?.label, '参考证据已附加')
  assert.match(summary?.detail ?? '', /1 条本轮真实 Markdown 来源/)
})
...
test('natural answer and Markdown locator contracts are wired across backend and UI', () => {
  ...
  assert.match(context, /不要输出 JSON、evidenceIds、\[E#\]/)
